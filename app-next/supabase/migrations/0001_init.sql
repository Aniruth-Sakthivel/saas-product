...
create index idx_reservations_dates on reservations(organization_id, check_in, check_out);
...
create index idx_stays_org on stays(organization_id);
create trigger trg_stays_updated before update on stays
  for each row execute function set_updated_at();
...
create index idx_hk_org on housekeeping_tasks(organization_id);
create index idx_hk_status on housekeeping_tasks(organization_id, status);
create trigger trg_hk_updated before update on housekeeping_tasks
  for each row execute function set_updated_at();
